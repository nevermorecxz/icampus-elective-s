...
public class CreateExcel {
	...
	public static void createExcelByClass(ResultsDTOByClass result,
	...
	public static void createExcelByCourses(ResultsDTOByCourses result,
			HttpServletResponse response) throws IOException {
		HSSFWorkbook workbook = new HSSFWorkbook();
		HSSFSheet sheet = workbook.createSheet("预选结果");
		...
		HSSFRow row = sheet.createRow(0);
		row.setHeight((short) 400);
		sheet.addMergedRegion(new CellRangeAddress(0, 0, 0, result.getData()
				.get(0).getData().size() - 1));
		HSSFCell ce1_1 = row.createCell(0);
		ce1_1.setCellValue(result.getTitle());
		ce1_1.setCellStyle(headerStyle);
		// 主要内容
		for (int y = 0; y < result.getData().size(); y++) {
			...
			for (int x = 0; x < result.getData().get(0).getData().size(); x++) {
				...
				if (y == 0) {
					...
					ce_y_x.setCellValue(result.getData().get(y).getData()
							.get(x));
				}
	...
	private static void downloadExcel(String title, HSSFWorkbook workbook,
			HttpServletResponse response) throws IOException {
		/* 生成excel */
	...
	public static void createExcelByCourse(ResultsDTOByCourses result,
			HttpServletResponse response) throws IOException {
		HSSFWorkbook workbook = new HSSFWorkbook();
		HSSFSheet sheet = workbook.createSheet("预选结果");
